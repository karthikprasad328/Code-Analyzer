...
// Author:      Author:      Author:Karthik Nanjundaswamy Guru Prasad(SUID:994344418) //
//              Syracuse University                                                   //
 ...
 * --------------------
 * ver 2.3 : 26 Sep 2014
 * -Added rules to detect relationships. 
 * -Added build2() function to create a parser with rules for relationship analysis.
 * -Created new classes Elem2 and Elem3
 * ver 2.2 : 24 Sep 2011
...
namespace CodeAnalysis
{
    ...
    //Action to calculate complexity
    public class FoundComplexity : AAction
    {
        Repository repo_;

        public FoundComplexity(Repository repo)
        {
            repo_ = repo;
        }
        public override void doAction(CSsemi.CSemiExp semi)
        {
            Elem elem = repo_.locations[repo_.locations.Count - 1];

            if (string.Compare(elem.type, "function") == 0)
            {
                for (int i = 0; i < repo_.stack.count; i++)
                {
                    if (string.Compare(repo_.stack[i].type, "function") == 0)
                    {
                        repo_.stack[i].scopecount = repo_.stack[i].scopecount + 1;
                        break;
                    }
    ...
    ///////////////////////////////////////////////////////////////
    /// Rule to calculate compleity
    public class DetectComplexity : ARule
    {
        public static bool isSpecialCase(CSsemi.CSemiExp semi)
        {
            string[] SpecialToken = { "if", "else", "switch", "for", "foreach", "while", "try", "catch","" };
            foreach (string token in SpecialToken)
            {
                if (semi.Contains(token) != -1)
                {
                    return true;
                }
        ...
        public class DetectUsing : ARule
        {
            ...
            public class BuildCodeAnalyzer
            {
                ...
                public virtual Parser build()
                {
                    ...
                    DetectComplexity complex = new DetectComplexity();
                    FoundComplexity complexity = new FoundComplexity(repo);
